...
for dir1 in `$eos_cmd ls ${eos_dir}`; do
	...
	echo ""
		...
		continue
    		...
    		for rootFile in `$eos_cmd ls ${eos_dir}$dir1/$dir2/$dir3/$dir4`; do

		    if test "${rootFile#*.root*}" == "$rootFile"; then
			continue
		    fi

    		    echo "Removing existing ${eos_dir}$dir1/$dir2/$dir3/$dir4/$rootFile"
		    `$eos_cmd rm ${eos_dir}$dir1/$dir2/$dir3/$dir4/$rootFile`

		done # End loop over root files
